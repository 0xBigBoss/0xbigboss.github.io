...
Run the one-command triage (from the ghostty repo where this skill was originally used):

```bash
# Original location in ghostty repo:
python .claude/skills/ghostty-crash-triage/scripts/triage.py

# Or if using this archived copy:
python drafts/ghostty-crash-triage-skill/scripts/triage.py
```
...
Ghostty uses GitHub Discussions as the first line of reporting for bugs and crashes. The triage script generates a `REPORT.md` skeleton in the output directory with crash details. You can then adapt this into a GitHub Discussion.
...
The output directory (e.g., `bug-crash-reports/2025-12-25/`) will contain the generated `REPORT.md`.

### Creating via GitHub CLI

After editing `REPORT.md` with your analysis, create the discussion using the GraphQL API:

```bash
# Read the report content
BODY=$(cat bug-crash-reports/<date>/REPORT.md)
